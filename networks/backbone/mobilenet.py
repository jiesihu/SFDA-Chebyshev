...
class MobileNetV2_BraTs(nn.Module):
    def __init__(self, output_stride=8, BatchNorm=None, width_mult=1., pretrained=True):
        super(MobileNetV2_BraTs, self).__init__()
        block = InvertedResidual
        input_channel = 32
        current_stride = 1
        rate = 1
    ...
    def _load_pretrained_model(self):
        ...
        first_conv = model_dict['features.0.0.weight'].mean(1,keepdim = True)
#         first_conv = torch.cat([first_conv,first_conv,first_conv,first_conv],dim = 1)
        model_dict['features.0.0.weight'] = first_conv
